...
<template>
  ...
  >
    <div class="gantt-elastic__calendar" :style="{ ...root.style['calendar'], width: root.state.options.width + 'px' }">
      ...
      <calendar-row :items="dates.minutes" which="minute" v-if="root.state.options.calendar.minute.display"></calendar-row> -->
      <calendar-row :items="dates.seconds" which="second" v-if="root.state.options.calendar.second.display"></calendar-row>
      <calendar-row :items="dates.milliseconds" which="millisecond" v-if="root.state.options.calendar.millisecond.display"></calendar-row>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    /**
     * How many milliseconds will fit? (NEW By TransferThought)
     *
     * @returns {object}
     */
    howManyMillisecondsFit(dayIndex) {
      const stroke = 1;
      const additionalSpace = stroke + 2;
      let fullCellWidth = this.root.state.options.times.steps[dayIndex].width.px;
      let formatNames = Object.keys(this.root.state.options.calendar.millisecond.format);
      for (let hours = 1000; hours > 1; hours = Math.ceil(hours / 2)) {
        for (let formatName of formatNames) {
          if (
            (this.root.state.options.calendar.millisecond.maxWidths[formatName] + additionalSpace) * hours <= fullCellWidth &&
            hours > 1
          ) {
            return {
              count: hours,
    ...
    /**
     * How many seconds will fit? (NEW By TransferThought)
     *
     * @returns {object}
     */
    howManySecondsFit() {
      ...
      let formatNames = Object.keys(this.root.state.options.calendar.second.format);
      for (let days = this.root.state.options.times.steps.length; days > 1; days = Math.ceil(days / 2)) {
        for (let formatName of formatNames) {
          if (
            (this.root.state.options.calendar.second.maxWidths[formatName] + additionalSpace) * days <= fullWidth &&
            days > 1
          ) {
            return {
              count: days,
    ...
    /**
     * How many minutes will fit? (NEW By TransferThought)
     *
     * @returns {object}
     */
    howManyMinutesFit(dayIndex) {
      const stroke = 1;
      const additionalSpace = stroke + 2;
      let fullCellWidth = this.root.state.options.times.steps[dayIndex].width.px;
      let formatNames = Object.keys(this.root.state.options.calendar.minute.format);
      for (let hours = 60; hours > 1; hours = Math.ceil(hours / 2)) {
        for (let formatName of formatNames) {
          if (
            (this.root.state.options.calendar.minute.maxWidths[formatName] + additionalSpace) * hours <= fullCellWidth &&
            hours > 1
          ) {
            return {
              count: hours,
    ...
    howManyMonthsFit() {
     ...
     /**
     * Generate Milliseconds (NEW by TransferThought)
     *
     * @returns {array}
     */
    generateMilliseconds() {
      ...
      for (let hourIndex = 0, len = steps.length; hourIndex < len; hourIndex++) {
        const hoursCount = this.howManyMillisecondsFit(hourIndex);
        if (hoursCount.count === 0) {
          continue;
        }
        const hours = { key: hourIndex + 'step', children: [] };
        const hourStep = 1000 / hoursCount.count;
        const hourWidthPx = steps[hourIndex].width.px / hoursCount.count;
        for (let i = 0, len = hoursCount.count; i < len; i++) {
          const hour = i * hourStep;
          let index = hourIndex;
          if (hourIndex > 0) {
            index = hourIndex - Math.floor(hourIndex / 1000) * 1000;
          }
          let textWidth = 0;
          if (typeof this.root.state.options.calendar.millisecond.widths[index] !== 'undefined') {
            textWidth = this.root.state.options.calendar.millisecond.widths[index][hoursCount.type];
          }
          ...
          hours.children.push({
            ...
            width: hourWidthPx,
            textWidth,
            height: this.root.state.options.calendar.millisecond.height,
            label: this.root.state.options.calendar.millisecond.formatted[hoursCount.type][hour]
          });
        }
        allHours.push(hours);
      }
    ...
    /**
     * Generate Seconds (NEW by TransferThought)
     *
     * @returns {array}
     */
    generateSeconds() {
      ...
      const daysCount = this.howManySecondsFit();
      ...
      for (let dayIndex = 0, len = steps.length; dayIndex < len; dayIndex += dayStep) {
        ...
        let textWidth = 0;
        if (typeof this.root.state.options.calendar.second.widths[dayIndex] !== 'undefined') {
          textWidth = this.root.state.options.calendar.second.widths[dayIndex][daysCount.type];
        }
        ...
        days.push({
          ...
          width: dayWidthPx,
          textWidth,
          height: this.root.state.options.calendar.second.height,
          label: this.root.state.options.calendar.second.format[daysCount.type](date.locale(localeName))
        });
      }
    ...
    /**
     * Generate Minutes (NEW by TransferThought)
     *
     * @returns {array}
     */
    generateMinutes() {
      ...
      const localeName = this.root.state.options.locale.name;
      for (let hourIndex = 0, len = steps.length; hourIndex < len; hourIndex++) {
        const hoursCount = this.howManyMinutesFit(hourIndex);
        if (hoursCount.count === 0) {
          continue;
        }
        const hours = { key: hourIndex + 'step', children: [] };
        const hourStep = 60 / hoursCount.count;
        const hourWidthPx = steps[hourIndex].width.px / hoursCount.count;
        for (let i = 0, len = hoursCount.count; i < len; i++) {
          const hour = i * hourStep;
          let index = hourIndex;
          if (hourIndex > 0) {
            index = hourIndex - Math.floor(hourIndex / 60) * 60;
          }
          let textWidth = 0;
          if (typeof this.root.state.options.calendar.minute.widths[index] !== 'undefined') {
            textWidth = this.root.state.options.calendar.minute.widths[index][hoursCount.type];
          }
          let x = steps[hourIndex].offset.px + hourWidthPx * i;
          hours.children.push({
            index: hourIndex,
            ...
            width: hourWidthPx,
            textWidth,
            height: this.root.state.options.calendar.minute.height,
            label: this.root.state.options.calendar.minute.formatted[hoursCount.type][hour]
          });
        }
        allHours.push(hours);
      }
  ...
  computed: {
    dates() {
      const milliseconds = this.generateMilliseconds();
      const seconds = this.generateSeconds();
      // const minutes = this.generateMinutes();
      const hours = this.generateHours();
      const days = this.generateDays();
      const months = this.generateMonths();

      const allDates = { milliseconds, seconds, hours, days, months };
