...
export default {
  ...
  mounted() {
    const d = new Date();
    this.newTime = d.getTime(); 
    
    this.$nextTick(() => {
      this.$nextTick(() => {
        // because of stupid slider :/
        this.root.scrollToTime(this.timeLinePosition.time);
        const cursorStep = (timestamp) => {
          if(this.root.options.play) {
            this.root.options.timelineCursor = timestamp - this.startTime;
          } else {
            this.startTime = timestamp - this.root.options.timelineCursor;
          }
          window.requestAnimationFrame(cursorStep);
        };
        window.requestAnimationFrame(cursorStep);
      });
    });
  ...
  computed: {
    ...
    timeLinePosition() {
      const d = new Date();
      const current = this.root.options.timelineCursor;
