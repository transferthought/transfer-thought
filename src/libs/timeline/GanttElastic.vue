  ...
  methods: {
    ...
    fillTasks(tasks) {
      for (let task of tasks) {
        ...
        //   task.startTime = task.start;//dayjs(task.start).valueOf();
        // }
    ...
    timeToPixelOffsetX(ms) {
      let x = ms - 0; //this.state.options.times.firstTime;
      if (x) {
    ...
    pixelOffsetXToTime(pixelOffsetX) {
      ...
      return offset * this.state.options.times.timePerPixel + 0;//this.state.options.times.firstTime;
    },
     ...
     */
    initTimes() {
      this.state.options.times.firstTime = 0;//dayjs(this.state.options.times.firstTaskTime)
        // .locale(this.state.options.locale.name)
        // .startOf('day')
        // .subtract(this.state.options.scope.before, 'days')
        // .startOf('day')
        // .valueOf();
      this.state.options.times.lastTime = 5 * 60000;//dayjs(this.state.options.times.lastTaskTime)
        // .locale(this.state.options.locale.name)
        // .endOf('day')
    ...
    computeCalendarWidths() {
      this.computeMillisecondWidths();
      this.computeSecondWidths();
      this.computeMinuteWidths();
    ...
    /**
     * Compute width of calendar seconds column widths basing on text widths (NEW by TransferThought)
     */
    computeMillisecondWidths() {
      const style = { ...this.style['calendar-row-text'], ...this.style['calendar-row-text--millisecond'] };
      ...
      let maxWidths = this.state.options.calendar.millisecond.maxWidths;
      if (maxWidths.length) {
        return;
      }
      for (let formatName in this.state.options.calendar.millisecond.format) {
        maxWidths[formatName] = 0;
      }
      for (let hour = 0; hour < 1000; hour++) {
        let widths = { hour };
        for (let formatName in this.state.options.calendar.millisecond.format) {
          const hourFormatted = this.state.options.calendar.millisecond.format[formatName](currentDate);
          this.state.options.calendar.millisecond.formatted[formatName].push(hourFormatted);
          widths[formatName] = this.state.ctx.measureText(hourFormatted).width;
        }
        this.state.options.calendar.millisecond.widths.push(widths);
        for (let formatName in this.state.options.calendar.millisecond.format) {
          if (widths[formatName] > maxWidths[formatName]) {
            maxWidths[formatName] = widths[formatName];
          }
        }
        currentDate = currentDate.add(1, 'millisecond');
      }
    },
     ...
     */
    computeSecondWidths() {
      const style = { ...this.style['calendar-row-text'], ...this.style['calendar-row-text--second'] };
      this.state.ctx.font = style['font-size'] + ' ' + style['font-family'];
      const localeName = this.state.options.locale.name;
      let currentDate = dayjs('2018-01-01T00:00:00').locale(localeName); // any date will be good for hours
      let maxWidths = this.state.options.calendar.second.maxWidths;
      if (maxWidths.length) {
        return;
      }
      for (let formatName in this.state.options.calendar.second.format) {
        maxWidths[formatName] = 0;
      }
      for (let hour = 0; hour < 60; hour++) {
        let widths = { hour };
        for (let formatName in this.state.options.calendar.second.format) {
          const hourFormatted = this.state.options.calendar.second.format[formatName](currentDate);
          this.state.options.calendar.second.formatted[formatName].push(hourFormatted);
          widths[formatName] = this.state.ctx.measureText(hourFormatted).width;
        }
        this.state.options.calendar.second.widths.push(widths);
        for (let formatName in this.state.options.calendar.second.format) {
          if (widths[formatName] > maxWidths[formatName]) {
            maxWidths[formatName] = widths[formatName];
          }
        }
        currentDate = currentDate.add(1, 'second');
      }
    },

    /**
     * Compute width of calendar minutes column widths basing on text widths (NEW by TransferThought)
     */
    computeMinuteWidths() {
      const style = { ...this.style['calendar-row-text'], ...this.style['calendar-row-text--minute'] };
      this.state.ctx.font = style['font-size'] + ' ' + style['font-family'];
      const localeName = this.state.options.locale.name;
      let currentDate = dayjs('2018-01-01T00:00:00').locale(localeName); // any date will be good for hours
      let maxWidths = this.state.options.calendar.minute.maxWidths;
      if (maxWidths.length) {
        return;
      }
      for (let formatName in this.state.options.calendar.minute.format) {
        maxWidths[formatName] = 0;
      }
      for (let hour = 0; hour < 60; hour++) {
        let widths = { hour };
        for (let formatName in this.state.options.calendar.minute.format) {
          const hourFormatted = this.state.options.calendar.minute.format[formatName](currentDate);
          this.state.options.calendar.minute.formatted[formatName].push(hourFormatted);
          widths[formatName] = this.state.ctx.measureText(hourFormatted).width;
        }
        this.state.options.calendar.minute.widths.push(widths);
        for (let formatName in this.state.options.calendar.minute.format) {
          if (widths[formatName] > maxWidths[formatName]) {
            maxWidths[formatName] = widths[formatName];
          }
    ...
    prepareDates() {
      ...
      this.state.options.times.firstTime = 0;//dayjs(firstTaskTime)
        // .locale(this.state.options.locale.name)
        // .startOf('day')
        // .subtract(this.state.options.scope.before, 'days')
        // .startOf('day')
        // .valueOf();
      this.state.options.times.lastTime = 60000;//dayjs(lastTaskTime)
        // .locale(this.state.options.locale.name)
        // .endOf('day')
        // .add(this.state.options.scope.after, 'days')
        // .endOf('day')
    ...
    visibleTasks() {
      ...
      for (let index = 0; index < len; index++) {
        ...
        for(let value of values){
          value.x = this.timeToPixelOffsetX(value.time);
          if (task.type === 'audio' || task.type === 'animation') {
            value.width = this.timeToPixelOffsetX(value.duration);
            if (isNaN(value.width) || value.width < 15) {
              value.width = 15;
            }
          } else {
            value.width = 15;
