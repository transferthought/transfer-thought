            ...
            <v-item-group v-model="selectedIndexes" :multiple="multiple">
                <v-row class="ma-0">
                    <v-col v-for="(item, index) in folders" :key="index" cols="3">
                        <v-item v-slot="{}">
                            <v-card outlined style="border-width: thick" @click="addSubFolder(item.name)">
                ...
                <v-row class="ma-0">
                    ...
                    <v-col v-if="loading" cols="3">
                        <v-card outlined style="border-width: thick">
...
export default {
    ...
    computed: {
        src() {
            return `private/${this.$store.state.user.user.username}`;
        },
        breadcrumbItems() {
    ...
    methods: {
        ...
        createFileSystem() {
            ...
            const add = async (source, target, item) => {
                ...
                if (elements.length) {
                    ...
                    const file = {
                        name: source,
                        thumbnail: this.getFileSrc(item),
        ...
        getFileSrc(file) {
            // const signedUrl = await Client.Storage.get(file.key, { level: 'private' });
            // const rawUrlParts = signedUrl.split('?');
            // const rawUrl = rawUrlParts[0];
            // return rawUrl;
