        ...
        <v-row no-gutters>
            ...
            <v-col v-if="selectedScene" cols="2" class="entity-edditor-container">
                ...
                <v-list>
                    ...
                    <v-expansion-panels :value="selectedInformationPointIndex" focusable accordion flat @change="handleSelectedInformationPointChanged">
            ...
            <v-card>
                ...
                <AssetViewer class="asset-viewer-content">
                    <template v-slot:AssetActions="{ selectedAsset }">
                        <v-spacer />
                        <v-btn color="primary" :disabled="!selectedAsset" @click="updateSelectedScenarioUrl(selectedAsset)">
                            Select
                        </v-btn>
                    </template>
                </AssetViewer>
...
export default {
    ...
    methods: {
        ...
        async updateSelectedScenarioUrl(selectedAsset) {
            if (this.selectedScene && selectedAsset) {
                await this.$store.dispatch('apps/state/update', {
                    selector: `scenes[${this.selectedSceneIndex}].url`,
