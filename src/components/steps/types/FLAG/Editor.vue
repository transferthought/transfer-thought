    ...
    <StepOverlayLayout>
        <template v-slot:Toolbar>
            ...
            <v-dialog v-model="showHotspotDialog" transition="dialog-top-transition" width="800">
                <v-card>
                    ...
                    <v-card-actions class="justify-end">
                        <v-btn text large class="text-h4 font-weight-400 mb-0 text-capitalize" @click="cancel">
                            Cancel
                        </v-btn>
                        <v-btn depressed color="primary" class="text-h4 font-weight-600 mb-0 text-capitalize" large @click="addHotspot">
                            Insert
                        </v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <SelectedEntityOptions
                ...
                @scaleUp="handleScaleEntity(0.1)"
                @scaleDown="handleScaleEntity(-0.1)"
                @flip="handleFlipEntity"
            />
        </template>
    </StepOverlayLayout>
</template>
...
import SelectedEntityOptions from '../SET_ENVIRONMENT/components/SelectedEntityOptions.vue';
...
export default {
    ...
    methods: {
        ...
        handleFlipEntity() {
            ...
            const hotspot = _.find(options, { id });
            if (hotspot && hotspot.scale) {
                const newScale = new THREE.Vector3(hotspot.scale.x * -1, hotspot.scale.y, hotspot.scale.z);
                this.updateOptionKey(id, 'scale', newScale);
            }
        },
        handleScaleEntity(scaleMultiple) {
            ...
            if (hotspot && hotspot.scale) {
                ...
                const newScale = new THREE.Vector3(hotspot.scale.x, hotspot.scale.y, hotspot.scale.z).multiplyScalar(scaler);
                this.updateOptionKey(id, 'scale', newScale);
            }
        },
    },
