 ...
 */
module.exports.Component = AFRAME.registerComponent('tt-look-controls', {
    ...
    init: function() {
        ...
        this.animateRestingCamera = false;
        this.lerpingCamera = false;
    ...
    setRotation: function(x, y, lerp) {
        if (this.pitchObject && this.yawObject) {
            let newX = THREE.MathUtils.degToRad(x);
            // do not let user put camera upside down
            newX = Math.max(-PI_2, Math.min(PI_2, newX));
            const newY = THREE.MathUtils.degToRad(y);
            const animateRestingCameraCopy = this.animateRestingCamera;
            if (animateRestingCameraCopy) {
                this.setAnimation(false);
            }

            if (lerp) {
                this.lerpingCamera = true;
                const coords = { x: this.pitchObject.rotation.x, y: this.yawObject.rotation.y };
                new TWEEN.Tween(coords)
                    .to({ x: newX, y: newY }, 300)
                    .onUpdate(() => {
                        this.pitchObject.rotation.x = coords.x;
                        this.yawObject.rotation.y = coords.y;
                    })
                    .onComplete(() => {
                        this.lerpingCamera = false;
                        // restart animation if it was supposed to be animating
                        if (animateRestingCameraCopy) {
                ...
                this.yawObject.rotation.y = newY;
                // restart animation if it was supposed to be animating
                if (animateRestingCameraCopy) {
                    this.setAnimation(true);
                }
    ...
    setAnimation: function(value) {
        // this.animateRestingCamera = value;
        // if (this.lerpingCamera && this.animateRestingCamera) {
        //   return;
        // }
        // if (this.pitchObject && this.yawObject) {
        //   if (value) {
        //     const delta = .1;
        //     const coords = { x: this.pitchObject.rotation.x, y: this.yawObject.rotation.y - delta };
        //     this.cameraAnimation = new TWEEN.Tween(coords)
        //       .to({ x: coords.x, y: coords.y + delta }, 3000)
        //       .onUpdate(() => {
        //         this.pitchObject.rotation.x = coords.x;
        //         this.yawObject.rotation.y = coords.y;
        //       })
        ...
        //       .yoyo(true)
        //       .start();
        //   } else if (!value && this.cameraAnimation) {
    ...
    onTouchMove: function(evt) {
        ...
        if (!this.magicWindowControls || !this.magicWindowControls.enabled) {
            pitchObject.rotation.x += deltaX * 0.15 * direction;
            pitchObject.rotation.x = Math.max(-PI_2, Math.min(PI_2, pitchObject.rotation.x));
        }
