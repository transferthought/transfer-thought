...
class HTMLCanvas {
        ...
        // Don't update if we are manipulating DOM for render
            ...
            for (let i = 0; i < mutationsList.length; i++) {
                ...
                if (!mutationsList[i].target.styleRef || mutationsList[i].attributeName == 'class') {
                    const styleRef = this.csssig(mutationsList[i].target);
    ...
    embedCss(url, css) {
        return new Promise((resolve) => {
            ...
            while (found = regEx.exec(css)) {
                promises.push(
                    this.getDataURL(new URL(found[1], url)).then((found => (url) => {
    ...
    generatePageCSS() {
        ...
        const elements = Array.from(document.querySelectorAll("style, link[type='text/css'],link[rel='stylesheet']"));
        const promises = [];
        for (let i = 0; i < elements.length; i++) {
            ...
            if (this.cssgenerated.indexOf(element) == -1) {
                ...
                if (element.tagName == 'STYLE') {
                    promises.push(
                        this.embedCss(window.location, element.innerHTML).then(((element, idx) => (css) => {
                            this.cssembed[idx] = css;
                        })(element, idx)),
                    );
                } else {
                    // Get embeded version of externally link stylesheets
                    promises.push(this.getURL(element.getAttribute('href')).then((idx => (xhr) => {
                        const css = new TextDecoder('utf-8').decode(xhr.response);
                        return this.embedCss(window.location, css).then(((element, idx) => (css) => {
                            this.cssembed[idx] = css;
                        })(element, idx));
                    })(idx)));
                }
    ...
    embededSVG() {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (link) {
                promises.push(this.getDataURL(link).then((element => (dataURL) => {
            ...
            if (elements[i].tagName == 'IMG' && elements[i].src.substr(0, 4) != 'data') {
                promises.push(this.getDataURL(elements[i].src).then((element => (dataURL) => {
            ...
            if (elements[i].namespaceURI == 'http://www.w3.org/1999/xhtml' && elements[i].hasAttribute('style')) {
                ...
                promises.push(
                    this.embedCss(window.location, style).then(((style, element) => (css) => {
        ...
        for (var i = 0; i < styles.length; i++) {
            promises.push(
                this.embedCss(window.location, styles[i].innerHTML).then((style => (css) => {
    ...
    updateFocusBlur() {
        const allElements = this.html.querySelectorAll('*');
        for (let i = 0; i < allElements.length; i++) {
            const element = allElements[i];
            if (element.tabIndex > -1) {
                if (!element.hasOwnProperty('focus')) {
                    element.focus = (element => () => this.setFocus(element))(element);
                }
                if (!element.hasOwnProperty('blur')) {
                    element.blur = (element => () => (this.focusElement == element ? this.setBlur() : false))(element);
                }
            } else {
    ...
    getParents() {
        ...
        do {
            ...
            const open = `<${tag == 'body' ? 'body xmlns="http://www.w3.org/1999/xhtml"' : tag} style="transform: none;left: 0;top: 0;position:static;display: block" class="${parent.className}"${parent.id ? ` id="${parent.id}"` : ''}>`;
            opens.unshift(open);
            const close = `</${tag}>`;
            closes.push(close);
            if (tag == 'body') break;
    ...
    updateCheckedAttributes() {
        ...
        for (let i = 0; i < inputElements.length; i++) {
            ...
            } else if (element.checked) element.setAttribute('checked', '');
        }
    }
    ...
    transformPoint(elementStyles, x, y, offsetX, offsetY) {
        ...
        const inverse = new THREE.Matrix4().getInverse(transform);
    ...
    getBorderRadii(element, style) {
        const properties = ['border-top-left-radius', 'border-top-right-radius', 'border-bottom-right-radius', 'border-bottom-left-radius'];
        let result;
        // Parse the css results
        const corners = [];
        for (var i = 0; i < properties.length; i++) {
            const borderRadiusString = style[properties[i]];
    ...
    clearHover() {
        ...
        if (this.lastEle) {
            this.lastEle.dispatchEvent(new MouseEvent('mouseleave', {
                bubbles: true,
                cancelable: true,
            }));
        }
